...
describe('aggregateProductCosts', () => {
  test('aggregates rows in stable product order with display labels', () => {
    const rows: ProcessedData[] = [
      ...
      makeProcessedData({ ...PRODUCT_COST_ROW_DEFAULTS, product: 'code_quality', sku: 'code_quality_ai_credit', model: 'Claude Sonnet 4.6', requestsUsed: 51.28584, grossAmount: 0.5128584, netAmount: 0.5128584 }),
    ...
    expect(aggregateProductCosts(rows)).toEqual([
      ...
      expect.objectContaining({ category: 'Code Quality', label: 'Code Quality', requests: 51.28584 }),
