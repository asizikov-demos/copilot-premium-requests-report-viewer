...
Rules:
- Run `npm run build`, `npm run lint`, and `npm run test` once before starting commits to verify no TypeScript, ESLint, or test errors — if any command fails, **abort immediately** and return the error to the caller (do NOT attempt to fix anything)
...
2. If this is the first commit in a batch, run the build/lint/test gate: `npm run build && npm run lint && npm run test` — if any step fails, abort and report the failure back (do NOT fix issues)
